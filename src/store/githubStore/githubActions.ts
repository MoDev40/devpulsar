...
import { GitHubRepository, GitHubIssue, TaskCategory, TaskPriority } from '@/types';
import { GitHubStore } from './githubTypes';
import { useTaskStore } from '@/store/taskStore';

export const createGitHubActions = (
  set: (state: Partial<GitHubStore> | ((state: GitHubStore) => Partial<GitHubStore>)) => void,
  get: () => GitHubStore
) => {
  return {
    connectGitHub: async () => {
    ...
    trackRepository: async (repository: GitHubRepository, trackIssues: boolean, trackPullRequests: boolean) => {
      const { user } = useAuthStore.getState();
    ...
    createTaskFromIssue: async (issue: GitHubIssue, category: string, priority: string) => {
